...
// 改编自 reactbits.dev 的 Ghost Fibers（WebGL 光纤场背景，依赖轻量库 ogl，无商业限制）。
// 与官方原版算法逐行一致，未做任何算法修改；lightMode 默认开启，贴合应用其他
// 页面的白底玻璃蓝主题：白色背底 + 主题主色光纤核（#0B60EA）+ 浅蓝柔光带（#93C5FD）。
const hexToRgb = (hex) => {
...
void main() {
  ...
  // 配色改成玻璃蓝主题：背底从原版的紫黑换成深藏蓝（#0A1424，色相与主题 primary 一致）
  vec3 backdrop = mix(vec3(0.039216, 0.078431, 0.141176), vec3(1.0), step(0.5, uLightMode));
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uGrain: { value: 0.05 },
        uLightMode: { value: 1 },
        uLineColor: { value: new Float32Array(hexToRgb('#0B60EA')) },
        uGlowColor: { value: new Float32Array(hexToRgb('#93C5FD')) },
      },
    });
