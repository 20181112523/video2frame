...
macOS 安装包**未经 Apple 官方签名**（签名需要付费的 Apple Developer Program 账号，$99/年），所以首次打开时系统会提示"XXX已损坏，无法打开，你应该将它移到废纸篓"。**这不是文件真的损坏**，是 macOS Gatekeeper 对未签名应用的默认拦截，按下面任一方法即可正常打开：

**方法一：终端命令（推荐，一次性）**
```bash
xattr -cr /Applications/Video2Frame.app
```
（如果是拖到其他目录，把路径换成实际安装位置）
