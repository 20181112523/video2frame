...
### 1. 芯片架构：目前只支持 Apple Silicon（M系列），不支持 Intel

CI 用的是 GitHub 的 `macos-latest` runner，目前默认是 arm64（M系列芯片），打出来的 `.dmg`/`.zip` 也是 arm64 版本。**Intel芯片的 Mac（2020 年之前的大部分机型）无法运行这个包**，双击会直接报错或者无法打开。

判断芯片型号：点屏幕左上角苹果图标 →「关于本机」，看"芯片"/"处理器"一栏：
- 显示 `Apple M1`/`M2`/`M3`/`M4` 等 → 用现有的 arm64 包，没问题
- 显示 `Intel` → 现有包用不了，需要额外构建 x64 版本（可以找开发者加一份 CI 配置）

### 2. "应用已损坏，无法打开"提示：未签名，不是真的损坏

**`.dmg` 和 `.zip` 两种格式都会遇到这个问题**，因为签名状态是打包时决定的，跟解压格式无关。

原因：macOS 安装包**未经 Apple 官方签名**（签名需要付费的 Apple Developer Program 账号，$99/年）。从网上下载的文件会被系统加上"隔离标记"，没有合法签名的应用会被 Gatekeeper 拦截，弹出"XXX已损坏，无法打开，你应该将它移到废纸篓"的提示。**这不是文件真的损坏**，按下面任一方法即可正常打开：

**方法一：终端命令（推荐，一次性）**
```bash
xattr -cr /Applications/Video2Frame.app
```
（如果是解压 `.zip` 到其他目录、或者拖到其他位置，把路径换成实际所在位置，比如 `~/Desktop/Video2Frame.app`）

**方法二：系统设置手动放行**
1. 双击应用，出现"已损坏"提示后点"好"关闭
2. 打开「系统设置」→「隐私与安全性」，往下滚动到底部，会看到"Video2Frame 已被阻止使用"的提示
3. 点击「仍要打开」，输入密码确认
...
两者内容完全一样（同一次构建产出），选哪个都行：
- `.dmg`：双击挂载后拖到「应用程序」文件夹，更接近标准 macOS 安装体验
- `.zip`：直接解压得到 `.app`，可以放在任意位置运行，适合不想安装、只是临时试用的场景
